...
def fix(rules_dir, profiles_dir=None, profile_name=None,
    ...
    for rule in rules:
        ...
        check_ok, check_detail = run_check(rule, timeout=check_timeout)
        ...
        # Step 2: run fix
        fix_ok, fix_detail = run_fix(rule, timeout=fix_timeout)
        if fix_ok:
            # Step 3: verify fix worked
            verify_ok, verify_detail = run_check(rule, timeout=check_timeout)
            if verify_ok:
                entry["status"] = "fixed"
                fixed_count += 1
            else:
